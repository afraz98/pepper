...
import { faPepperHot, faPencil, faDumpsterFire } from "@fortawesome/free-solid-svg-icons"
  ...
  renderItems = () => {
    ...
    return newItems.map((item) => (
      <li key={item.id} className="list-group-item d-flex justify-content-between align-items-center">
        ...
        <span>
          <button className="btn btn-secondary mr-2" onClick={() => this.editItem(item)}> <FontAwesomeIcon icon={faPencil}/> </button>
  ...
  render() {
    return (
      <main className="container">
        <div>
          <h1 className="text-black text-uppercase text-center my-4">Pepper <FontAwesomeIcon icon={faPepperHot}/></h1>
        </div>
        <div className="row">
