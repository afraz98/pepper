...
class App extends Component {
  ...
  handleSubmit = (item) => {
    ...
    if (item.id) {
      axios.put(`/api/issues/${item.id}/`, item).then((res) => this.refreshList());
      return;
    }
    axios.post("/api/issues/", item).then((res) => this.refreshList());
  };

  handleDelete = (item) => {
    axios
      .delete(`/api/issues/${item.id}/`)
  ...
  render() {
    return (
      <main className="container">
        <h1 className="text-white text-uppercase text-center my-4">Pepper</h1>
        <div className="row">
          <div className="col-md-6 col-sm-10 mx-auto p-0">
            <div className="card p-3">
              <div className="mb-4"><button className="btn btn-primary" onClick={this.createItem}> Add task </button></div>
              {this.renderTabList()}
